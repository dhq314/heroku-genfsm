-module(erlshell_action_jsonp_resource).
-export([
    init/1,
    allowed_methods/2,
...
erlshell_create(ReqData) ->
    ...
    case erlshell_server:start_link([ProcessName, ?HEART_TIME_INTERVAL]) of
        {ok, _Pid} ->
            ...
            [{result, 1}, {action, 1}, {pid, ProcessName}, {interval, ?HEART_TIME_INTERVAL}, {line_num, 1}, {start_time, StartTime}, {client_ip, ReqData#wm_reqdata.peer}];
        _ ->
            [{result, 2}, {action, 1}]
    end.
